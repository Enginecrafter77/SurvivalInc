...
public class SchopClientSounds {
	...
	/*
	 * Client-only sound handling Mainly for insanity, ambiance, etc.
	 */
	...
	public static void playSound(String uuid, String soundMethod, double posX, double posY, double posZ)
	{
		...
		if (player.getCachedUniqueIdString().equals(uuid) && player.world.isRemote)
		{
			...
			// appear.
			if (soundMethod.equals("EndermanSound"))
			{
				playEndermanSound(world, pos);
			}
			else if (soundMethod.equals("ZombieSound"))
			{
				playZombieSound(world, pos);
			}
			else if (soundMethod.equals("GhastSound"))
			{
				playGhastSound(world, pos);
			}
			else if (soundMethod.equals("ExplosionSound"))
			{
				playExplosionSound(world, pos);
			}
			else if (soundMethod.equals("StoneBreakSound"))
			{
				playStoneBreakSound(world, pos);
			}
			else if (soundMethod.equals("FireSound"))
			{
				playFireSound(world, pos);
			}
			else if (soundMethod.equals("VillagerSound"))
			{
				playVillagerSound(world, pos);
			}
			else if (soundMethod.equals("LavaSound"))
			{
				playLavaSound(world, pos);
			}
			else if (soundMethod.equals("InsanityAmbienceSound"))
			{
				playInsanityAmbienceSound(world, pos);
			}
			else if (soundMethod.equals("InsanityAmbienceSoundLoud"))
			{
				playInsanityAmbienceSoundLoud(world, pos);
			}
		}
	}
	...
	// Lava sounds blub blub pop plop plob blub bluh well actually just the
	// ambient noises.
