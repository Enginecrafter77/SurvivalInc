...
public class SchopServerParticles {
	...
	/*
	 * Responsible for spawning particles for various crap... server-side. So
	 * everyone can see them. Client-side is another file, used for sanity, so
	 * players will think they're insane when they are. Efficient.
	 */
	...
	public static void summonParticle(String uuid, String particleMethod, double posX, double posY, double posZ)
	{
		...
		for (int num = 0; num < playerCount; num++)
		{
			...
			if (player.getCachedUniqueIdString().equals(uuid) && !player.world.isRemote)
			{
				...
				// Determine what particles need to be
				// summoned/spawned/rendered/i used a million ways to describe
				...
				{
					spawnDrinkWaterParticles(serverWorld, posX, posY, posZ);
				}
				else if (particleMethod.equals("SweatParticles"))
				{
					spawnSweatParticles(serverWorld, posX, posY, posZ);
				}
				else if (particleMethod.equals("ColdBreathParticles"))
				{
					spawnColdBreathParticles(serverWorld, posX, posY, posZ);
				}
				else if (particleMethod.equals("GhostParticles"))
				{
					spawnGhostParticles(serverWorld, posX, posY, posZ);
				}
				else if (particleMethod.equals("ResurrectionFlameParticles"))
				{
					spawnResurrectionFlameParticles(serverWorld, posX, posY, posZ);
				}
				else if (particleMethod.equals("ResurrectionEnchantmentParticles"))
				{
					spawnResurrectionEnchantmentParticles(serverWorld, posX, posY, posZ);
				}
				else if (particleMethod.equals("TowelWaterParticles"))
				{
					spawnTowelWaterParticles(serverWorld, posX, posY, posZ);
				}
	...
	private static void spawnWetnessParticles(WorldServer serverWorld, double posX, double posY, double posZ,
			float wetness)
	{
		...
		if (wetness >= 10.0f)
		{
			serverWorld.spawnParticle(EnumParticleTypes.DRIP_WATER, posX, posY, posZ, wetnessRounded, 0.3d, 0.5d, 0.3d,
					10.0d, null);
		}
	}

	// Spawn water particles if a player drinks from a water block directly with
	// their bare hands.
	private static void spawnDrinkWaterParticles(WorldServer serverWorld, double posX, double posY, double posZ)
	{
		...
		if (posX != 0.0d && posY != 0.0d && posZ != 0.0d)
		{
			...
			serverWorld.spawnParticle(EnumParticleTypes.WATER_BUBBLE, posX + randOffset, posY, posZ + randOffset, 20,
					0.5d, 1d, 0.5d, 0.1d, null);
			serverWorld.spawnParticle(EnumParticleTypes.WATER_SPLASH, posX + randOffset, posY + 1, posZ + randOffset,
					50, 0.2d, 0.5d, 0.2d, 0.05d, null);
		}
	}

	// Spawn some sweat particles when the player is hot enough. Don't take it
	// THAT way.
	...
	// Spawn some cool snow particles to imitate a person's breath, when they're
	// in a cold biome.
	...
	// Spawn some weird ghost particles to make a ghost visible (or what those
	// ghost hunting shows call it: manifest themselves)
	...
	private static void spawnResurrectionFlameParticles(WorldServer serverWorld, double posX, double posY, double posZ)
	{
		...
		serverWorld.spawnParticle(EnumParticleTypes.FLAME, posX + 0.5, posY + 1.75, posZ + 3.5, 0, 0, 0.5, -1, 0.15,
				null);
		serverWorld.spawnParticle(EnumParticleTypes.FLAME, posX + 0.5, posY + 1.75, posZ - 2.5, 0, 0, 0.5, 1, 0.15,
				null);
		serverWorld.spawnParticle(EnumParticleTypes.FLAME, posX + 3.5, posY + 1.75, posZ + 0.5, 0, -1, 0.5, 0, 0.15,
				null);
		serverWorld.spawnParticle(EnumParticleTypes.FLAME, posX - 2.5, posY + 1.75, posZ + 0.5, 0, 1, 0.5, 0, 0.15,
				null);
	}

	// Spawn enchantment particles during resurrection
	private static void spawnResurrectionEnchantmentParticles(WorldServer serverWorld, double posX, double posY,
			double posZ)
	{
