...
public class LeavesChanger {
	...
	// around the player, changing the leaves.
	// We'll use the same method as SnowMelter.melt(), except with all rows, not
	// just a random one.
		...
		else
		{
			...
			for (int x = 0; x < 16; x++)
			{
				...
				for (int z = 0; z < 16; z++)
				{
					...
					// New BlockPos; get the top-most y-value. Then go down one,
					// because precipitation height is one block above the
					// ground.
					pos = chunk.getPrecipitationHeight(pos);
					pos = pos.down();

					// Loop through each y-level below until it's not leaves, or
					// the column isn't worth changing.
					for (int y = 0; y < 20; y++)
					{
	...
	private boolean tryToChangeSingleBlock(World world, BlockPos pos, Season season)
	{
		...
		if (state.getBlock() == Blocks.LEAVES)
		{
			...
			if (season == Season.AUTUMN)
			{
				...
				if (state.getBlock() instanceof BlockOldLeaf)
				{
					...
					if (state.getValue(BlockOldLeaf.VARIANT) == EnumType.BIRCH
					...
					else if (state.getValue(BlockOldLeaf.VARIANT) == EnumType.OAK
							&& state.getValue(BlockLeaves.DECAYABLE).booleanValue())
					{
				...
				else
				{
					...
					// Ehh no. Not enough metadata. Perhaps with 1.13, when
					// metadata is gonna disappear. Oh dear.
					return false;
				}
			}
		...
		else if (state.getBlock() == Blocks.LEAVES2)
		{
			...
			if (season == Season.AUTUMN)
			{
				...
				// Okay. This block needs to change. What type of leaves? Just
				// in case
		...
		else if (state.getBlock() == ModBlocks.RED_LEAVES && state.getValue(BlockLeaves.DECAYABLE).booleanValue())
		{
			...
			if (season == Season.SPRING)
			{
				...
				world.setBlockState(pos, Blocks.LEAVES.getDefaultState().withProperty(BlockLeaves.DECAYABLE, true)
						.withProperty(BlockLeaves.CHECK_DECAY, false).withProperty(BlockOldLeaf.VARIANT, EnumType.OAK));
				return true;
			}
		...
		else if (state.getBlock() == ModBlocks.YELLOW_LEAVES && state.getValue(BlockLeaves.DECAYABLE).booleanValue())
		{
			...
			if (season == Season.SPRING)
			{
				...
				// Alrighty. Change them back.
				world.setBlockState(pos,
						Blocks.LEAVES.getDefaultState().withProperty(BlockLeaves.DECAYABLE, true)
								.withProperty(BlockLeaves.CHECK_DECAY, false)
								.withProperty(BlockOldLeaf.VARIANT, EnumType.BIRCH));
				return true;
			}
		...
		else if (state.getBlock() == ModBlocks.ORANGE_LEAVES && state.getValue(BlockLeaves.DECAYABLE).booleanValue())
		{
			...
			if (season == Season.SPRING)
			{
				...
				// Alrighty. Change them back.
				world.setBlockState(pos,
						Blocks.LEAVES2.getDefaultState().withProperty(BlockLeaves.DECAYABLE, true)
								.withProperty(BlockLeaves.CHECK_DECAY, false)
								.withProperty(BlockNewLeaf.VARIANT, EnumType.DARK_OAK));
				return true;
			}
