...
public class ThirstModifier {
	...
	public void onPlayerUpdate(EntityPlayer player)
	{
		...
		// Side effects of dehydration include fatigue and dizzyness. Those are
		// replicated here. Well, attempted.
		// =========================================================================================================

		// Does the player have existing attributes with the same name? Remove
		// them.
		// Iterate through all of modifiers. If one of them is a thirst one,
		...
		AttributeModifier attackSpeedDebuff = new AttributeModifier("thirstAttackSpeedDebuff", attackSpeedDebuffAmount,
				0);
	...
	public void onPlayerInteract(EntityPlayer player)
	{
		...
		RayTraceResult raytrace = player.world.rayTraceBlocks(player.getPositionEyes(1.0f),
			...
			if (raytrace.typeOfHit == RayTraceResult.Type.BLOCK)
			{
				...
				if (player.world.getBlockState(pos).getMaterial() == Material.WATER && handItems.next().isEmpty())
				{
					...
					else if (biome instanceof BiomeSwamp)
					{
						...
						SchopServerEffects.affectPlayer(player.getCachedUniqueIdString(), "poison", 12, 3, false,
								false);
					}
					else
					{
						...
						{ // 50% chance
							...
							SchopServerEffects.affectPlayer(player.getCachedUniqueIdString(), "poison", 12, 1, false,
									false);
						}
					}
