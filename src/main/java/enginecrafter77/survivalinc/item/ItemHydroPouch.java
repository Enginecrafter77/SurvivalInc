...
public class ItemHydroPouch extends Item {
	...
	public ItemHydroPouch()
	{
		...
		setRegistryName(new ResourceLocation(SurvivalInc.MOD_ID, "hydropouch"));
	...
	// Right click to get water. If the player is holding an empty canteen, it
	// will attempt to fill it up with water.
	// If holding a canteen partially filled with water, it will refill it with
	// water.
	// If holding a full canteen with the same type of water it's going to get,
	// it'll empty itself, becoming an empty canteen.
	@Override
		...
		if (!world.isRemote)
		{
			...
			if (raytrace != null && raytrace.typeOfHit == RayTraceResult.Type.BLOCK)
			{
						...
						|| world.getBlockState(pos).getBlock() == Blocks.FLOWING_WATER)
				{
					...
					// This is a bigger canteen, so take a water source block if
					// it's empty.
					...
					else if (biome instanceof BiomeSnow)
					{
						...
						// Random chance to give cold water opposed to dirty
						// water.
						...
						// Otherwise fill it with dirty water or cold water,
						// according to what's already in it.
						else if (heldItem.getMetadata() == 2 || heldItem.getMetadata() == 4)
						{
							heldItem.setItemDamage(2);
							sips = hydroPouchSips;
						}
					...
					else
					{
						...
						// Random chance to give fresh water opposed to dirty
						// water.
						double randomNum = Math.random();
					...
					SchopServerParticles.summonParticle(player.getCachedUniqueIdString(), "DrinkWaterParticles",
							pos.getX(), pos.getY(), pos.getZ());
					SchopServerSounds.playSound(player.getCachedUniqueIdString(), "WaterSound", pos.getX(), pos.getY(),
							pos.getZ());
				...
				else if (world.getBlockState(pos).getBlock() == Blocks.CAULDRON)
				{
					...
					if (cauldronLevel == 3 && world.isRainingAt(new BlockPos(pos.getX(), pos.getY() + 1, pos.getZ())))
					{
						...
						SchopServerParticles.summonParticle(player.getCachedUniqueIdString(), "DrinkWaterParticles",
								pos.getX(), pos.getY(), pos.getZ());
						SchopServerSounds.playSound(player.getCachedUniqueIdString(), "WaterSound", pos.getX(),
								pos.getY(), pos.getZ());
					}
				...
				// All of this crap is to ensure that the player can drink from
				// the canteen if it isn't empty.
	...
	public String getUnlocalizedName(ItemStack stack)
	{
		...
		{
			return "item." + SurvivalInc.RESOURCE_PREFIX + "empty_hydropouch";
		}
		else if (stack.getMetadata() == 1)
		{
			return "item." + SurvivalInc.RESOURCE_PREFIX + "fresh_water_hydropouch";
		}
		else if (stack.getMetadata() == 2)
		{
			return "item." + SurvivalInc.RESOURCE_PREFIX + "dirty_water_hydropouch";
		}
		else if (stack.getMetadata() == 3)
		{
			return "item." + SurvivalInc.RESOURCE_PREFIX + "salt_water_hydropouch";
		}
		else if (stack.getMetadata() == 4)
		{
			return "item." + SurvivalInc.RESOURCE_PREFIX + "filtered_water_hydropouch";
		}
		else if (stack.getMetadata() == 5)
		{
			return "item." + SurvivalInc.RESOURCE_PREFIX + "cold_water_hydropouch";
		}
		else
		{
			stack.setItemDamage(0);
	...
	// When crafted, give this item an NBT tag to store its number of sips
	// available, along with durability.
	@Override
