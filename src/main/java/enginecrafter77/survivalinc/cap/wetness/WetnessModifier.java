	...
	public void onPlayerUpdate(Entity player)
	{
		...
		else if (player.isWet())
		{
			...
			if (player.isInWater())
			{
				...
				// their feet. If so, just go to 40% wetness Or somewhere around
				// there.
				// We'll check if water is in the player's face. If so, 100%. If
				// not, 40%.
				if (player.world.getBlockState(posFace).getBlock() != Blocks.WATER
						&& player.world.getBlockState(posFace).getBlock() != Blocks.FLOWING_WATER)
				{

					if (wetness.getWetness() < 40)
					{
						wetness.increase(1.25f * modifier);
					}
				}
		...
		else
		{
			...
			// Figure out the conditions of the world, then dry off naturally
			// accordingly.
			if (player.world.isDaytime() && player.world.canBlockSeeSky(pos))
			{
				wetness.decrease(0.01f * modifier);
			}
			else
			{
				wetness.decrease(0.005f * modifier);
			}
		}
		...
		// These if-statement blocks is for stuff that directly doesn't have to
		// do with water bombardment.
		...
		if (ProximityDetect.isBlockNextToPlayer(playerPosX, playerPosY, playerPosZ, Blocks.FIRE, player))
		{
			...
			if (player.isWet())
			{
				wetness.decrease(0.25f * modifier);
			}
			else
			{
				wetness.decrease(0.5f * modifier);
			}
		}

		// Check if the player is near a fire - two blocks away. if there's a
		// block between the player and the fire, it won't count.
		else if (ProximityDetect.isBlockNearPlayer2(playerPosX, playerPosY, playerPosZ, Blocks.FIRE, player, false))
		{
			...
			if (player.isWet())
			{
				wetness.decrease(0.15f * modifier);
			}
			else
			{
				wetness.decrease(0.25f * modifier);
		...
		else if (ProximityDetect.isBlockUnderPlayer(playerPosX, playerPosY, playerPosZ, Blocks.FIRE, player))
		{
			...
			if (player.isWet())
			{
				wetness.decrease(0.20f * modifier);
			}
			else
			{
				wetness.decrease(0.30f * modifier);
			}
		}
		...
		else if (ProximityDetect.isBlockUnderPlayer2(playerPosX, playerPosY, playerPosZ, Blocks.FIRE, player, false))
		{
			...
			if (player.isWet())
			{
				wetness.decrease(0.10f * modifier);
			}
			else
			{
				wetness.decrease(0.20f * modifier);
		...
		else if (ProximityDetect.isBlockNearPlayer2(playerPosX, playerPosY, playerPosZ, Blocks.LAVA, player, false))
		{
			...
			{
				wetness.decrease(0.45f * modifier);
			}
		}
		...
		// Check if the player is near lava - two blocks away.
		else if (ProximityDetect.isBlockNearPlayer2(playerPosX, playerPosY, playerPosZ, Blocks.FLOWING_LAVA, player,
				false))
		{
		...
		// ...And two blocks.
		else if (ProximityDetect.isBlockUnderPlayer2(playerPosX, playerPosY, playerPosZ, Blocks.FLOWING_LAVA, player,
				false))
		{
		...
		if (ProximityDetect.isBlockNextToPlayer(playerPosX, playerPosY, playerPosZ, Blocks.LIT_FURNACE, player))
		{
			...
			{
				wetness.decrease(0.40f * modifier);
			}
		}

		// Two blocks.
		else if (ProximityDetect.isBlockNearPlayer2(playerPosX, playerPosY, playerPosZ, Blocks.LIT_FURNACE, player,
				false))
		{
			...
			if (player.isWet())
			{
				wetness.decrease(0.05f * modifier);
			}
			else
			{
				wetness.decrease(0.20f * modifier);
		...
		SchopServerParticles.summonParticle(player.getCachedUniqueIdString(), "WetnessParticles", doublePlayerPosX,
				doublePlayerPosY, doublePlayerPosZ);
	}
}