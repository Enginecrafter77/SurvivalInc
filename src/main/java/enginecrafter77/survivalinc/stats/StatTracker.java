...
public interface StatTracker {
	...
	/**
	 * Attempts to unregister the target provider from this tracker instance. If there is no such provider registered, this
	...
	/**
	 * Searches for a {@link StatProvider provider} with the specified ID. If no such provider is found, null is returned by
	 * this method.
	 *
	...
	/**
	 * Assigns or replaces a {@link StatRecord} to the specified {@link StatProvider}.
	 *
	 ...
	 * null as the player parameter, in which case the method only checks whether the stat has been suspended from updating
	 * or not.
	 *
	...
	/**
	 * Suspends or resumes the specified stat. Suspending a stat causes the stat to skip running it's provider
	 ...
	 * passing <i>null</i> as player parameter to {@link #isActive(StatProvider, EntityPlayer)}.
	 *
